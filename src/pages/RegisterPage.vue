  ...
  <div>
    ...
    >
      ...
      <v-btn
        ...
        variant="tonal"
        block
        @click="_registerWithGithub"
      >
        <v-icon class="me-2" size="large"> mdi-github </v-icon>
        Signup with Github
      </v-btn>
      <v-btn
        class="mb-4"
        ...
        variant="tonal"
        block
        @click="_registerWithGoogle"
      >
        <v-icon class="me-2" size="large"> mdi-google </v-icon>
        Signup with google
      </v-btn>
    ...
    .catch(async (error) => {
      switch (error.code) {
        ...
        case "auth/email-already-in-use":
          var isExistsThisCompany = await checkCompanyUser(
            email.value,
            companyId
          )
          if (isExistsThisCompany) {
            ...
            if (await _existUser(email.value, "company_users")) {
              await signOut(auth)
              return (errMsg.value =
                "This is company account, you can not access with this user.")
            } else {
